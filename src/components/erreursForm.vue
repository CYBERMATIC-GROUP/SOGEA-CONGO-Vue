...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    getError(
      (error as any).response?.data.fault.detail.includes("Que s'est-il passé")
        ? "Le numéro de téléphone existe déjà, c'est un champs unique"
        : (error as any).response?.data.fault.detail
    );
    loading.value = false;
  }
});
