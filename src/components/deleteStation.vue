...
const getPropietaire = useStation();
...
const Delete = async () => {
  loading.value = true;
  try {
    let response = await getPropietaire.deleteStation(data.value.IDSTATIONS);
    console.log(response);
    loading.value = false;
    updateopenDelete(false);
    await RefrehFunction();
    getSuccess("La station a été supprimer avec succèss");
  } catch (error) {
    console.error(error);
    loading.value = false;
