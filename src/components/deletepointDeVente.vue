<template>
  <DialogContent class="min-w-[30rem] px-10">
    <div>
      <DialogTitle>Voulez-vous supprimer cet élément ? jude</DialogTitle>
    </div>
...
const getPropietaire = usePointDeVente();
...
const Delete = async () => {
  loading.value = true;
  try {
    let response = await getPropietaire.deletePointDeVente(
      data.value.IDVENDEURS
    );
    console.log(response);
    loading.value = false;
    updateopenDelete(false);
    await RefrehFunction();
    getSuccess("Le point de vente a été supprimer avec succèss");
  } catch (error) {
    console.error(error);
    loading.value = false;
